...
TEST(PDIFEditOp, TestConstructor) {
    ...
    ASSERT_NO_THROW({pdif::edit_op op(pdif::edit_op_type::META, pdif::stream_elem::create<pdif::text_elem>("Hello, World!"));});
    op = pdif::edit_op(pdif::edit_op_type::META, pdif::stream_elem::create<pdif::text_elem>("Hello, World!"));

    ASSERT_EQ(op.get_type(), pdif::edit_op_type::META);
    ASSERT_EQ(op.has_arg(), true);
    ASSERT_EQ(op.get_arg()->type(), pdif::stream_type::text);
    ASSERT_EQ(op.get_arg()->as<pdif::text_elem>()->text(), "Hello, World!");
}
...
TEST(PDIFEditOp, TestConstructorInvalidArg) {
    ...
    ASSERT_THROW({pdif::edit_op op(pdif::edit_op_type::META);}, pdif::pdif_invalid_argment);

    ASSERT_THROW({pdif::edit_op op(pdif::edit_op_type::META, std::nullopt);}, pdif::pdif_invalid_argment);
}
...
TEST(PDIFEditOp, TestExecuteMeta) {
    pdif::stream s;
