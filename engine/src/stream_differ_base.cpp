...
void stream_differ_base::meta_diff(pdif::diff& d) {
    auto stream1_meta = stream1.get_metadata();
    auto stream2_meta = stream2.get_metadata();    

    for (auto& meta : stream2_meta) {
        if (!stream1.has_key(meta.first)) {
        ...
        if (stream1.has_key(meta.first)) {
            ...
            if (stream1.get_metadata(meta.first) != stream2.get_metadata(meta.first)) {
    ...
    for (auto &meta : stream1_meta) {
        if (!stream2.has_key(meta.first)) {
